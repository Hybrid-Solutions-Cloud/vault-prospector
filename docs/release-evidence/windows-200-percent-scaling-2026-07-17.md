...
This evidence closes only the internal 200% display-scaling defect. It does not prove NVDA or Narrator behavior, Windows High Contrast, text-only scaling, target-size conformance, complete keyboard-only tasks, focus return from Entra or Windows Hello, usability with representative users, a packaged MSI, or independent accessibility review.
