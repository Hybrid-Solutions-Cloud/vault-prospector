...
| Authenticated cache decisions | Envelope v2 included descriptor metadata as AES-GCM associated data, but retrieval applied expiry/fingerprint decisions before verifying the tag and scoped purge trusted the unauthenticated vault/workspace descriptor. Tampering could influence deletion or evade a scoped purge, although it could not release plaintext. | Retrieval authenticates the complete envelope before applying item, expiry, or fingerprint decisions. Scoped purge derives the requested item from the published filename, authenticates the envelope before trusting its scope, removes malformed/tampered/unknown entries conservatively, and continues through valid entries. Tests cover nonce/tag/ciphertext and descriptor tampering, expiry manipulation, cross-item substitution, missing fields, malformed JSON/Base64/tag length, legacy/future versions, and purge continuation. |

## Verification

- The 2026-07-17 locked Release gate passed structured direct/transitive vulnerability inspection,
  formatting verification, a 0-warning/0-error build, and 100/100 tests across all seven projects.
