...
| P-17 | Support, troubleshooting, vulnerability response, and rollback runbooks | Yes | In progress | The security policy now provides a private HCS contact, response targets, and withdrawal rules. User, authentication, and release-verification docs exist. GitHub private vulnerability reporting, package rollback, credential rotation, failed-update recovery, and incident procedures remain open. |
