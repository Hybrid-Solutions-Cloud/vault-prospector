...
| P-02 | Locked restore, formatting, warnings-as-errors build, and automated tests | Yes | Passed | Local Release verification on 2026-07-16: locked restore, structured vulnerability inspection, and formatting passed; build completed with 0 warnings and 0 errors; 57 tests passed. CI repeats these gates on `main`. |
...
| P-11 | Chocolatey package validation and public repository acceptance | Yes | Blocked | The tested package, recorded checksum, and immutable public asset have the same SHA-256. Four authenticated submission attempts through 2026-07-17 UTC returned Chocolatey HTTP 504; the fourth followed a fresh anonymous asset download and hash verification. The exact OData endpoint still returned 404 and exact pre-release CLI search remained empty at 2026-07-17 01:30 UTC, proving the version was not ingested. Retry only after push-service recovery, then complete moderation. |
...
| P-15 | Accessibility and core-task usability | Yes | In progress | A clean 1024x768 Windows 11 run found that Preview.2's `1180x760` default clipped controls. The unreleased source candidate now opens centered at `960x640`; its detail pane scrolls to all six actions and Windows Hello guidance. A self-contained build was launched through the real Explorer desktop and Hyper-V input proved reachability. Command-state tests and earlier keyboard/contrast evidence remain green. Target sizes, 200% scaling/reflow, High Contrast, NVDA/Narrator, full keyboard paths, and structured usability testing remain. See the [preliminary accessibility audit](../release-evidence/accessibility-audit-2026-07-16.md) and [clean Windows 11 validation](../release-evidence/clean-windows-11-validation-2026-07-17.md). |
