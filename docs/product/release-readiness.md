...
| P-15 | Accessibility and core-task usability | Yes | In progress | A clean 1024x768 Windows 11 run found that Preview.2's `1180x760` default clipped controls. The unreleased source candidate now opens centered at `960x640`; its detail pane scrolls to all six actions and Windows Hello guidance. A self-contained build was launched through the real Explorer desktop and Hyper-V input proved reachability. Every text-entry, selector, list, and numeric control now has an explicit automation name, application status is a polite live region, and regression tests enforce those semantics. Command-state tests and earlier keyboard/contrast evidence remain green. Target sizes, 200% scaling/reflow, High Contrast, NVDA/Narrator, full keyboard paths, and structured usability testing remain. See the [preliminary accessibility audit](../release-evidence/accessibility-audit-2026-07-16.md) and [clean Windows 11 validation](../release-evidence/clean-windows-11-validation-2026-07-17.md). |
