...
**Current promotion target:** `0.1.1-preview.1`, an explicitly unsigned Windows evaluation release.
Public CI.68 remains available for testing until the immutable Preview assets publish;
...
| P-02 | Locked restore, formatting, warnings-as-errors build, and automated tests | Yes | Passed | Local Release verification on 2026-07-17: locked restore, structured direct/transitive vulnerability inspection, and formatting passed; build completed with 0 warnings and 0 errors; all seven test projects passed 111/111 tests. Exact-commit CI run [29592049330](https://github.com/Hybrid-Solutions-Cloud/vault-prospector/actions/runs/29592049330) repeated the build/test/package gates and full-history secret scan successfully. |
...
| P-09 | Clean-machine MSI install, upgrade, repair, uninstall, and retained-data behavior | Yes | Passed | On 2026-07-17, the exact locally packaged `0.1.1-preview.1` MSI passed all 27 lifecycle gates on isolated Windows 11: immutable Preview.2 install; deterministic post-`InstallFiles` failure returning `1603` with byte-identical prior registration/files/shortcut/state rollback; successful upgrade to MSI `0.1.101`; forced repair; downgrade rejection; uninstall cleanup; and retained encrypted-state preservation. The VM was restored with zero registrations/processes and the test root removed. See [`0.1.1-preview.1` evidence](../release-evidence/0.1.1-preview.1.md), [failed-upgrade rollback validation](../release-evidence/windows-installer-failed-upgrade-2026-07-17.md), and [clean Windows 11 validation](../release-evidence/clean-windows-11-validation-2026-07-17.md). |
...
| P-12 | Immutable public artifacts, checksums, SBOM, and provenance | Yes | In progress | Corrected CI build [`0.1.0-ci.68`](https://github.com/Hybrid-Solutions-Cloud/vault-prospector-releases/releases/tag/v0.1.0-ci.68) is published with checksum and machine-readable provenance; an anonymous download matched SHA-256 `C605DF2135F81EBF5B19B14F846A50A41801C8D945A1CCF419F08831B9A3546E`. Passing P-12 requires `0.1.1-preview.1` MSI, ZIP, NUPKG, WinGet bundle, checksums, SPDX SBOM, Sigstore bundles, classification, and provenance from the protected tag workflow to publish immutably and pass anonymous revalidation. |
...
| P-18 | Preview go/no-go record | Yes | In progress | The `0.1.1-preview.1` [go/no-go record](../release-evidence/0.1.1-preview.1-go-no-go.md) identifies the named approver, accepted non-production risks, rollback triggers, and evidence still required before tagging. Candidate hashes and exact release-workflow evidence remain pending. |
...
GA. P-09 is now passed. P-12 and P-18 are not accepted by declaration: exact tagged artifacts,
hash/provenance, anonymous download, and final decision evidence must complete before promotion.
