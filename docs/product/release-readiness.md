...
| P-15 | Accessibility and core-task usability | Yes | In progress | A clean 1024x768 Windows 11 run found that Preview.2's `1180x760` default clipped controls. The unreleased source candidate first reduced the default viewport, then a real 200% Windows run exposed its remaining 900x620 minimum as unusable. The candidate now fits the scaled work area and stacks Search, Identities, Workspaces, and their actions below 720 logical pixels. All five tabs and core empty-state actions were reached at an effective 512x360 logical work area, the first-run foreground was corrected, and the normal 100% layout was revalidated. Every text-entry, selector, list, and numeric control has an explicit automation name, application status is a polite live region, and regression tests enforce those semantics. Target sizes, text-only scaling, High Contrast, NVDA/Narrator, full keyboard paths, packaged-candidate retesting, structured usability, and independent sign-off remain. See the [preliminary accessibility audit](../release-evidence/accessibility-audit-2026-07-16.md), [200% scaling validation](../release-evidence/windows-200-percent-scaling-2026-07-17.md), and [clean Windows 11 validation](../release-evidence/clean-windows-11-validation-2026-07-17.md). |
