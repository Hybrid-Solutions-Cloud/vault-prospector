...
| P-02 | Locked restore, formatting, warnings-as-errors build, and automated tests | Yes | Passed | Local Release verification on 2026-07-16: locked restore, structured vulnerability inspection, and formatting passed; build completed with 0 warnings and 0 errors; 41 tests passed. CI repeats these gates on `main`. |
...
| P-15 | Accessibility and core-task usability | Yes | In progress | A preliminary first-run audit confirmed keyboard access to the dynamic custom-registration field, meaningful automation names, and passing contrast for the newly added onboarding/error colors. Full-task keyboard behavior, no-op/disabled action behavior, target sizes, 200% scaling, high contrast, NVDA/Narrator, and structured usability testing remain. See the [preliminary accessibility audit](../release-evidence/accessibility-audit-2026-07-16.md). |
