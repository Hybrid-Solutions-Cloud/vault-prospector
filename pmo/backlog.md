...
| Local login/unlock and MFA boundary | Partial | Windows Hello gates reveal/copy/cache operations; Entra enforces its own MFA | Dedicated app-unlock setup/recovery experience and full live Hello coverage |
| Mandatory local encryption | Implemented, review open | SQLCipher metadata plus DPAPI-protected key; AES-GCM encrypted opt-in offline values; no plaintext toggle | Independent security assessment, supported backup/key rotation/device migration |
...
| Managed-identity authentication | Not implemented | Requirements and security constraints only | Runtime detection, selection, credential flow, permissions, tests, UI |
| Service-principal authentication | Not implemented | Requirements and preference for certificate/federated credentials only | Credential model, secure configuration, rotation/revocation, tests, UI |
| List existing managed identities/SPNs | Not implemented | Acceptance criteria only | Authorized ARM/Graph discovery, eligibility and permission display, UI |
| Create a managed identity/SPN during setup | Not implemented | Acceptance criteria only | Dry run, creation, least-privilege RBAC, confirmation, audit, rollback |
| Discover accessible Key Vaults | Partial | Selected interactive identity enumerates visible subscriptions and ARM Key Vault resources, then lists accessible secret/key/certificate metadata | Setup wizard, explicit access-path selection, management/data-plane permission matrix |
| Read-only default | Implemented | No Key Vault mutation or Azure role-assignment operations exist | Policy UI explaining effective permissions per identity/vault |
...
| Notification-area/background operation | Not implemented | Requirements only | Tray lifecycle, lock states, close behavior, metadata-only background sync |
| Password-manager UI research/redesign | Not implemented | Requirements only; accessibility and scaling fixes were made to the existing UI | Research synthesis, prototypes, usability testing, selected redesign |
...
Status snapshot: 2026-07-17. **Delivered** means present in the current public Preview unless the
...
| 3.4 | Configure discovery inclusion | Not started | Phase 6 |
...
| 4.4 | Reconcile removed provider objects | Not started | Phase 7 |
...
| 6.3 | Purge cache | Delivered | Phases 3 and 7 completion |
...
| 9.4 | Human and workload identity choices | Partial; human only | Phase 4 |
| 9.5 | Discover and provision workload identities | Not started | Phase 5 |
| 10.1 | Discover vaults by selected access path | Partial | Phase 6 |
