...
GitHub Bug #97 / ADO AB#7310, with Tasks AB#7311–#7312, tracks the multi-tenant discovery defect
reported on 2026-08-12. Local source now enumerates every ARM tenant available to the selected
...
Entra-backed Windows user in Remote Desktop because the credential fallback can forward an
unqualified Entra account name to Windows without the required `AzureAD` authority. The local fix
retains exact current-SID matching and adds redacted outcome diagnostics; exact Preview 11 MSI,
...
| Local login/unlock and MFA boundary | Preview 10 Entra/RDP blocker; local fix pending package validation | Fail-closed app unlock and sensitive operations use Windows verification; the fix supplies the Entra authority only for an already Entra-backed current identity and retains exact SID matching | AB#7337 exact Preview 11 Entra/RDP validation, local/domain/Entra negative matrix, VDI/AVD coverage, Windows Hello/recovery coverage, and independent review |
