...
public sealed class EncryptedSqliteMetadataRepository(string databasePath, IKeyMaterialProvider keyMaterial) : IMetadataRepository
{
    ...
    public async Task UpsertIdentityAsync(ConnectedIdentity identity, CancellationToken cancellationToken)
    {
        ...
        await ExecuteAsync(connection, null, """
            INSERT INTO identities(id,client_id,account_identifier,username_hint,display_name,home_tenant_id,auth_state,last_interactive,is_enabled)
            VALUES($id,$client,$account,$username,$display,$tenant,$state,$last,$enabled)
            ON CONFLICT(id) DO UPDATE SET client_id=excluded.client_id,account_identifier=excluded.account_identifier,username_hint=excluded.username_hint,display_name=excluded.display_name,home_tenant_id=excluded.home_tenant_id,auth_state=excluded.auth_state,last_interactive=excluded.last_interactive,is_enabled=excluded.is_enabled
            """, cancellationToken,
    ...
    public async Task<IReadOnlyList<SearchResult>> SearchAsync(SearchRequest request, DateTimeOffset now, CancellationToken cancellationToken)
    {
        ...
        while (await reader.ReadAsync(cancellationToken))
        {
            ...
            results.Add(new SearchResult(item, vault, reader.GetString(23), reader.GetString(24), reader.GetBoolean(25), ReadDate(reader, 26), reader.GetBoolean(27)));
        }
        return results;
    }
    ...
    {
        await using var connection = await OpenAsync(cancellationToken);
        await ExecuteAsync(connection, null, "DELETE FROM workspaces WHERE id=$id", cancellationToken, ("$id", id.ToString("D")));
    }
    ...
    private static Task UpsertSubscriptionAsync(SqliteConnection c, SqliteTransaction t, SubscriptionAccess x, CancellationToken ct) => ExecuteAsync(c, t, "INSERT INTO subscriptions(id,tenant_access_id,subscription_id,display_name,state,is_selected,last_discovered) VALUES($id,$tenant,$subscription,$name,$state,$selected,$last) ON CONFLICT(id) DO UPDATE SET display_name=excluded.display_name,state=excluded.state,is_selected=excluded.is_selected,last_discovered=excluded.last_discovered", ct, ("$id", x.Id.ToString("D")), ("$tenant", x.TenantAccessId.ToString("D")), ("$subscription", x.SubscriptionId), ("$name", x.DisplayName), ("$state", x.State), ("$selected", x.IsSelected ? 1 : 0), ("$last", Format(x.LastDiscoveredAt)));
    ...
    private const string IdentitySelect = "SELECT id,client_id,account_identifier,username_hint,display_name,home_tenant_id,auth_state,last_interactive,is_enabled FROM identities";
    private const string SearchSql = """
        SELECT i.id,i.vault_id,i.name,i.object_type,i.enabled,i.tags,i.content_type,i.created_at,i.updated_at,i.expires_at,i.provider_version,i.fingerprint,i.last_indexed,i.is_deleted,
               ...
               CASE WHEN julianday($now)-julianday(i.last_indexed)>1 THEN 1 ELSE 0 END
    ...
    private const string ResolveSql = """
        SELECT i.id,i.vault_id,i.name,i.object_type,i.enabled,i.tags,i.content_type,i.created_at,i.updated_at,i.expires_at,i.provider_version,i.fingerprint,i.last_indexed,i.is_deleted,
               v.resource_id,v.name,v.tenant_id,v.subscription_id,v.resource_group,v.location,v.tags,v.vault_uri,v.last_indexed,
               ident.id,ident.client_id,ident.account_identifier,ident.username_hint,ident.display_name,ident.home_tenant_id,ident.auth_state,ident.last_interactive,ident.is_enabled
        FROM items i JOIN vaults v ON v.id=i.vault_id JOIN vault_access va ON va.vault_id=v.id JOIN identities ident ON ident.id=va.identity_id
    ...
    private const string Schema = """
        ...
        CREATE TABLE IF NOT EXISTS vault_access(id TEXT PRIMARY KEY,vault_id TEXT NOT NULL REFERENCES vaults(id) ON DELETE CASCADE,identity_id TEXT NOT NULL REFERENCES identities(id) ON DELETE CASCADE,tenant_id TEXT NOT NULL,status TEXT NOT NULL,last_validated TEXT NOT NULL,failure_category TEXT,preferred_rank INTEGER NOT NULL,UNIQUE(vault_id,identity_id,tenant_id));
