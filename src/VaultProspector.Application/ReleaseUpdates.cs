    ...
    IUpdateInstallerLauncher installerLauncher) : IReleaseUpdateService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        using var response = await httpClient.SendAsync(
            ...
            cancellationToken);
        ValidateExactResponseUri(response, ReleasesApiUri);
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (release.Availability != ReleaseUpdateAvailability.Available)
        {
            throw new InvalidOperationException(
                "Only a newer trusted release can be installed.");
        }
        ...
        var versionDirectory = Path.GetFullPath(
            Path.Combine(_updateDirectory, release.LatestVersion));
        EnsureContainedPath(_updateDirectory, versionDirectory);
        ...
        var installerPath = Path.GetFullPath(
            Path.Combine(versionDirectory, release.PackageName));
        EnsureContainedPath(versionDirectory, installerPath);
        ...
        try
        {
            ...
            using var response = await httpClient.SendAsync(
                ...
                HttpCompletionOption.ResponseHeadersRead,
                cancellationToken);
            ValidateAssetResponseUri(response, release.PackageUri);
            response.EnsureSuccessStatusCode();
        ...
        CancellationToken cancellationToken)
    {
        ...
        EnsureContainedPath(_updateDirectory, installerPath);
        if (!string.Equals(
        ...
        CancellationToken cancellationToken)
    {
        ...
        using var response = await httpClient.SendAsync(
            ...
            HttpCompletionOption.ResponseHeadersRead,
            cancellationToken);
        ValidateAssetResponseUri(response, uri);
        response.EnsureSuccessStatusCode();
    ...
    private static void ValidateReleaseInfo(ReleaseUpdateInfo release)
    {
        var version = ProductVersion.TryParse(release.LatestVersion);
        if (version is null ||
            !string.Equals(
                release.PackageName,
    ...
    private static void ValidateExactResponseUri(
        HttpResponseMessage response,
        Uri expectedUri)
    {
        var responseUri = response.RequestMessage?.RequestUri;
        if (responseUri is null || responseUri != expectedUri)
        {
            throw new InvalidDataException(
                "Release metadata redirected outside its exact trusted API endpoint.");
        }
    }

    private static void ValidateAssetResponseUri(
        HttpResponseMessage response,
        Uri requestedUri)
    {
        var responseUri = response.RequestMessage?.RequestUri;
        if (responseUri == requestedUri)
            return;
        ...
        {
            throw new InvalidDataException(
                "A release asset redirected outside GitHub's release-asset service.");
        }
    }
